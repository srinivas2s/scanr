...
<template>
  <div class="p-5 clay-card space-y-4 font-sans">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-12 gap-3.5">
      ...
      <div class="sm:col-span-4 flex items-center gap-2">
        ...
        >
          ...
          <span>{{ torchActive ? 'On' : 'Torch' }}</span>
        </IndustrialButton>
        ...
        >
          ...
          <svg v-else class="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15zM17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2" />
          </svg>
    ...
    <div v-if="devices.length > 1" class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-3 border-t border-slate-100 text-xs">
      <label for="camera-select" class="text-slate-600 font-semibold">
        Selected Camera:
      </label>
      <select
        id="camera-select"
        ...
        class="clay-inset text-slate-900 px-3.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-amber-400 text-xs cursor-pointer font-medium"
      >
